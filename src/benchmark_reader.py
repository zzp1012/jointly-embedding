...
class Benchmark:
    ...
    def unique_entry_mtriples(self):
        heads = [triple.s for entry in self.entries for triple in entry.modifiedtripleset.triples]
        tails = [triple.o for entry in self.entries for triple in entry.modifiedtripleset.triples]
        return set(heads + tails)

    def unique_mtriples(self):
        triples = []
        for entry in self.entries:
            for triple in entry.modifiedtripleset.triples:
                if triple not in triples:
                    triples.append((triple, entry.lexs))
        return triples
